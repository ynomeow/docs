...
В качестве подлючения используется демонстрационное подключение **Sample ClickHouse**.
...
1. Добавьте на карту координаты точек доставки. Для этого из раздела **Измерения** перетащите поле **DeliveryAdressCoord** в секцию **Геоточки (тепловая карта)**.
